...
# Original Contributers: Victor Vimbert-Guerlais (helloHackYnow)
# Recontribution by jackaopen:Added clearer instrucntion and added a alpha slider
import tkinter
...
class AskColor(customtkinter.CTkToplevel):
    ...
    def __init__(self,
        ...
        self.hex_color_with_alpha = "#ffffffff"
        ...
        self.color_instruction_label = customtkinter.CTkLabel(master=self.frame, text="Drag the cursor to select a color", text_color='white', bg_color=self.fg_color)
        self.color_instruction_label.pack(pady=(10, 0))
        ...
        self.brightness_label = customtkinter.CTkLabel(master=self.frame, text="Adjust Brightness", text_color='white', bg_color=self.fg_color)
        self.brightness_label.pack(pady=(0, 10))
        ...
        self.slider.pack(fill="both", pady=(0,5), padx=20-self.slider_border)

       
        
        self.alpha_instruction_label = customtkinter.CTkLabel(master=self.frame, text="Adjust Alpha (Transparency)", text_color='white', bg_color=self.fg_color)
        self.alpha_instruction_label.pack(pady=(5, 5))
        ...
        self.label = customtkinter.CTkLabel(master=self.frame, text_color="#000000", height=50, fg_color=self.default_hex_color,
                                            corner_radius=self.corner_radius*0.5, text=self.default_hex_color)
        ...
        self.brightness_slider_value.set(50)
        self.alpha_slider_value.set(30)
        self.update_colors()
        self.notification_label = customtkinter.CTkLabel(
            master=self.frame,
            text="💡 For best results, try lowering both Brightness"
                 " and Alpha to get softer, transparent tones.",
        ...
        self.notification_label.pack(fill="both", padx=10, pady=(5, 5))
    ...
    def get(self):
        self._color = self.hex_color_with_alpha
    ...
    def update_colors(self):
        ...
        self.hex_color_with_alpha = "#{:02x}{:02x}{:02x}{:02x}".format(r, g, b, alpha)

        self.slider.configure(progress_color=self.default_hex_color)
        self.alpha_slider.configure(progress_color=self.default_hex_color)

        # Blend color with background to simulate alpha visually in the label
        try:
            bg_r, bg_g, bg_b = self._hex_to_rgb(self.bg_color)
        except Exception:
            bg_r, bg_g, bg_b = 30, 30, 30
        a_ratio = alpha / 255
        blended_hex = "#{:02x}{:02x}{:02x}".format(
            int(r * a_ratio + bg_r * (1 - a_ratio)),
            int(g * a_ratio + bg_g * (1 - a_ratio)),
            int(b * a_ratio + bg_b * (1 - a_ratio)),
        )

        self.label.configure(fg_color=blended_hex)
        self.label.configure(text=self.hex_color_with_alpha)
        
        perceived_brightness = int(r * a_ratio + bg_r * (1 - a_ratio)) * 0.299 + \
                               int(g * a_ratio + bg_g * (1 - a_ratio)) * 0.587 + \
                               int(b * a_ratio + bg_b * (1 - a_ratio)) * 0.114
        self.label.configure(text_color="white" if perceived_brightness < 128 else "black")
