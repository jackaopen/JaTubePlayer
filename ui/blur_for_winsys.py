...
if platform.system() == "Windows":
    ...
    def blur(hwnd, hexColor: bool = False, Acrylic: bool = False, Dark: bool = False, disable: bool = False):
        if not disable:
            subprocess.run(["xprop", "-f", "_KDE_NET_WM_BLUR_BEHIND_REGION", "32c", "-set", "_KDE_NET_WM_BLUR_BEHIND_REGION", "0"], check=True)
        else:
            subprocess.run(["xprop", "-remove", "_KDE_NET_WM_BLUR_BEHIND_REGION"], check=True)
